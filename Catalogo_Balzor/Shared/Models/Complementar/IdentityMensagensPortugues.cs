...
namespace Catalogo_Balzor.Shared.Models.Complementar
{
    public class IdentityMensagensPortugues : IdentityErrorDescriber
    {
        ...
        {
            return new IdentityError { Code = nameof(InvalidUserName), Description = $"O login {userName} é invalido, pode conter apenas letras ou digitos." };
        }

        public override IdentityError InvalidEmail(string email)
        {
            return new IdentityError { Code = nameof(InvalidEmail), Description = $"O {email} é invalido." };
        }
        ...
        {
            return new IdentityError { Code = nameof(UserAlreadyInRole), Description = "O usuário já possui a permissão" };
        }

        public override IdentityError UserNotInRole(string role)
        {
            return new IdentityError { Code = nameof(UserNotInRole), Description = $"U usuário já possui a permissão {role}." };
        }
        ...
        {
            return new IdentityError { Code = nameof(PasswordRequiresNonAlphanumeric), Description = $"As senhas devem conter ao menos um caracter não alfanumérico." };
        }
        ...
        {
            return new IdentityError { Code = nameof(PasswordRequiresLower), Description = $"As senha deve conter ao menos um caracter em caixa baixa ('0'-'9')." };
        }

        public override IdentityError PasswordRequiresUpper()
        {
            return new IdentityError { Code = nameof(PasswordRequiresUpper), Description = $"As senhas deve conter ao menos um caracter em caixa alta ('A'-'Z')." };
        }
